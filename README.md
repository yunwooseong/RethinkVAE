...
VAE-based collaborative filtering models achieve competitive performance, but like other deep networks they become overconfident as training progresses. Two properties of collaborative filtering make this worse: the interaction matrix is extremely sparse, and implicit feedback treats every unobserved interaction as a negative. Under a likelihood-based objective the model pushes the predicted score of the overwhelming majority of pairs toward zero, the reconstruction loss for negatives vanishes, and so do its gradients. We show, theoretically and empirically, that this overconfidence causes *embedding collapse*: user and item embeddings fall into a low-rank subspace and lose representational capacity. To fill this gap, we study label smoothing (LS) in the VAE-based CF setting. Smoothing the interaction matrix as $X_{LS} = (1-r)\,X + \tfrac{r}{L}\mathbf{1}$ gives the reconstruction loss a strictly positive lower bound on negatives, keeping gradients alive and preventing collapse. Applied to PoisVAE, BiVAE and DualVAE, LS improves every metric on ML-1M, AKindle and Yelp, with up to 15% relative gains, restores full-rank latent spaces, and works across Poisson, Bernoulli and multinomial likelihoods. We further find that the optimal smoothing factor decreases with data sparsity, which offers practical guidance for choosing $r$.
