...
class DualVAE(nn.Module):
    ...
    def contrast_loss(self, x, x_,):
        ...
        # positive
        pos_score_aspect = torch.sum(torch.mul(x_, x), dim=-1)
        pos_score_aspect = torch.exp(pos_score_aspect / 0.2)
    ...
    def loss(self, x, x_, kl, kl_beta, cl, cl_gama, smoothing):
        # Likelihood
        if smoothing > 0: 
            x_smoothed = x * (1 - smoothing) + smoothing * 0.5
        else:  
            x_smoothed = x
        ll_choices = {
            "bern": x_smoothed * torch.log(x_ + EPS) + (1 - x_smoothed) * torch.log(1 - x_ + EPS),
            "gaus": -(x_smoothed - x_) ** 2,
            "pois": x_smoothed * torch.log(x_ + EPS) - x_,
            "mult": torch.log(x_ + EPS) * x_smoothed
        }
...
def learn(
    ...
    for epoch in progress_bar:
        ...
        for i_ids in train_set.item_iter(batch_size, shuffle=False):
            ...
            i_list.append(z_i_mu_list)

            i_loss = dualvae.loss(i_batch, probs, kl, beta_kl, cl, gama_cl, smoothing)
        ...
        for u_ids in train_set.user_iter(batch_size, shuffle=False):
            ...
            u_loss = dualvae.loss(u_batch, probs, kl, beta_kl, cl, gama_cl, smoothing)
